    ...
    override fun hentKjernInfoFraRestClient(orgnummer: String): OrganisasjonResponse {
        ...
        try {
            ...
            if (response.code() in 200..299) {
                TjenestekallLogger.info("orgnavn-response: $uuid", mapOf(
                        "status" to "${response.code()} ${response.message()}",
                        "body" to body
                ))
            } else {
                TjenestekallLogger.error("orgnavn-response-error: $uuid", mapOf(
                        "status" to "${response.code()} ${response.message()}",
                        "orgnummer" to orgnummer,
                        "body" to body
                ))
            ...
            log.error("Feilet ved post mot orgnavn (ID: $uuid)", exception)
            TjenestekallLogger.error("orgnavn-error: $uuid", mapOf(
