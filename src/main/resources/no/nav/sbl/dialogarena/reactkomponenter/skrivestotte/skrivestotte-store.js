const Utils = require('./../utils/utils-module');
const Store = require('./../utils/store');

const SkrivestotteStore = function SkrivestotteStore() {
...
const hentSokeresultater =
    Utils.debounce(function execSok(fritekst, knagger) {
        SkrivestotteStore._sok(fritekst, knagger).done(function oppdater(resultat) {
...
SkrivestotteStore.prototype.tekstChanged = function tekstChanged(tekst, tabliste) {
...
SkrivestotteStore.prototype.leggTilKnagg = function leggTilKnagg(knagg) {
...
SkrivestotteStore.prototype.slettKnagg = function slettKnagg(knagg) {
...
SkrivestotteStore.prototype.setLocale = function setLocale(locale) {
    this.state.valgtLocale = locale;
    this.fireUpdate(this.listeners);
};

SkrivestotteStore.prototype.onChange = function onChange(data) {
...
SkrivestotteStore.prototype.onKeyDown = function onKeyDown(tabliste, event) {
...
function autofullfor(tekst, autofullforMap) {
    ...
    return tekst.replace(/\[(.*?)]/g, function replacer(tekstIgnored, resultat) {
...
SkrivestotteStore.prototype.submit = function submit(onSubmit, event) {
    event.preventDefault();
    const $tekstfelt = $('#' + this.state.tekstfeltId);
    /*
    ...
    setTimeout(function IEHack() {
         ...
         */
        const IE9placeholder = 'Alt du skriver i denne boksen blir synlig for bruker når du trykker "Del med bruker".';
        let eksisterendeTekst = $tekstfelt.val();
...
SkrivestotteStore._sok = function _sok(fritekst = '', knagger = []) {
    const knaggfriFritekst = fritekst.replace(/^#*(.*)$/, '$1');

    let url = '/modiabrukerdialog/rest/skrivestotte/sok?fritekst=' + encodeURIComponent(knaggfriFritekst);
    if (knagger.length !== 0) {
...
export default SkrivestotteStore;
