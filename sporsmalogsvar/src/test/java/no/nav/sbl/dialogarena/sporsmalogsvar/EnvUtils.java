...
public class EnvUtils {
    public static void withSubject(String ident, Runnable fn) {
        Subject original = SubjectHandler.getSubjectHandler().getSubject();
        ...
        withEnv(new HashMap<String, String>(){{
            put(SubjectHandler.SUBJECTHANDLER_KEY, ThreadLocalSubjectHandler.class.getCanonicalName());
            put(SecurityConstants.SYSTEMUSER_USERNAME, "srvModiabrukerdialog");
        }}, fn);

        SubjectHandlerUtils.setSubject(original);
    }
