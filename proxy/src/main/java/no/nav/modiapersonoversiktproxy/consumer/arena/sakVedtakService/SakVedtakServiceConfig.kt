...
@Configuration
open class SakVedtakServiceConfig {
    private val log = LoggerFactory.getLogger(SakVedtakServiceConfig::class.java)
    val address: String = EnvironmentUtils.getRequiredProperty("ARENA_SAK_VEDTAK_URL")

    @Bean
    open fun sakVedtakPortType(): SakVedtakPortType {
    ...
    @Bean
    open fun sakVedtakService(sakVedtakPortType: SakVedtakPortType) = SakVedtakServiceImpl(sakVedtakPortType)
