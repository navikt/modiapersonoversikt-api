...
class VeiledersRollerPip(private val ldap: LDAPService) : Kabac.AttributeProvider<Set<String>> {
    ...
    override fun provide(ctx: EvaluationContext): Set<String> {
        return ldap.hentRollerForVeileder(ctx.requireValue(NavIdentPip)).toSet()
    }
}
