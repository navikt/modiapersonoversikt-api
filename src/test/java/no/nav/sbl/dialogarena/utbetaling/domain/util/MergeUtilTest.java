...
public class MergeUtilTest {
    ...
    public void kanMerge_MangeForskjelligeMergeables() throws Exception {
        ...
        List<Farge> blandedeFarger = merge(new ArrayList<Mergeable<Farge>>(asList(rod, gronn1, gul, gronn1, gronn2, gronn3)), Farge.MERGEABLE_NAVN, Farge.MERGEABLE_NAVN);

        List<String> navn = asList("Grønn", "Gul", "Rød");
        List<Integer> hues = asList(108, 60, 1);
        assertThat(blandedeFarger.size(), is(3));
        assertThat(blandedeFarger.get(0).navn, is(navn.get(0)));
        assertThat(blandedeFarger.get(0).hue, is(hues.get(0)));
        assertThat(blandedeFarger.get(1).navn, is(navn.get(1)));
        assertThat(blandedeFarger.get(1).hue, is(hues.get(1)));
        assertThat(blandedeFarger.get(2).navn, is(navn.get(2)));
        assertThat(blandedeFarger.get(2).hue, is(hues.get(2)));
    }
    ...
    private static final class Farge implements Mergeable<Farge> {
        ...
        public Farge doMerge(List<Mergeable> skalMerges) {
            ...
            Set<String> navneSet = on(farger).map(NAVN).collectIn(new HashSet<String>());
            String fargeNavn = join(navneSet, ", ");
