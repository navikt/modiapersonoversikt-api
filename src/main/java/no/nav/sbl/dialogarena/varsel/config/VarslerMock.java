    ...
    public WSHentVarslerResponse hentVarsler(WSHentVarslerRequest wsHentVarslerRequest) {
        ...
        return new WSHentVarslerResponse()
                .withVarselListe(new WSVarselListe().withVarsel(
                        ...
                        new WSVarsel()
                                .withVarseltype("Gruppeaktivitet")
                                .withMottattidspunkt(new XMLGregorianCalendarImpl(now.minusDays(3).toGregorianCalendar()))
                                .withStatus("Ferdig")
                                .withMeldingListe(new WSMeldingListe().withMelding(
                                        lagWsMelding("SMS", "Kom på gruppeaktivititet SMS", "12345678", "", "OK", now.minusDays(1), "", ""),
                                        lagWsMelding("EPOST", "Kom på gruppeaktivititet EPOST", "test@testesen.com", "", "OK", now.minusDays(1), "Dette er en invitasjon", ""),
                                        lagWsMelding("NAV.NO", "Kom på gruppeaktivititet NAVNO", "", "", "OK", now.minusDays(1), "", "http://www.nav.no/")
                                ))
                ));
    }
