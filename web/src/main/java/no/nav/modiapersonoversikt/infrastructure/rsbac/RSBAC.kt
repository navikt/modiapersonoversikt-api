package no.nav.modiapersonoversikt.infrastructure.rsbac

import no.nav.modiapersonoversikt.infrastructure.naudit.Audit
import no.nav.modiapersonoversikt.infrastructure.rsbac.DecisionEnums.DENY
import no.nav.modiapersonoversikt.infrastructure.rsbac.DecisionEnums.PERMIT
    ...
    override fun <S> get(auditDescriptor: Audit.AuditDescriptor<in S>, supplier: Supplier<S>): S {
        ...
        if (decision.value == PERMIT) {
            return runCatching(supplier)
                .onSuccess(auditDescriptor::log)
                .onFailure(auditDescriptor::failed)
                .getOrThrow()
